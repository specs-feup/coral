...
export default class ConstraintGenerator extends Pass {
    ...
    override _apply_impl($jp: Joinpoint): PassResult {
        const universal = this.regionck.regions.filter(r => r.kind === RegionVariable.Kind.UNIVERSAL);
        ...
        for (const node of this.regionck.cfg.graph.nodes()) {
            ...
            for (const variable of scratch.liveIn.keys()) {
                ...
                if (ty === undefined) {
                    throw new Error(`ConstraintGenerator: variable ${variable} not found in declarations`);
                }
    ...
    #reborrowConstraints(node: cytoscape.NodeSingular, successor: string) {
        ...
        for (const path of scratch.loan.loanedPath.supportingPrefixes()) {
            ...
            this.#relateRegions(path.regionvar, (scratch.loan as Loan).regionVar, Variance.CONTRA, successor);                
        }
    }
    ...
    #relateRegions(region1: RegionVariable, region2: RegionVariable, variance: Variance, successor: string) {
        switch (variance) {
